...
class GeneralCalculator(BasicTaskCalculator):
    ...
    def compute_loss(self, model, data, adj_low_un, adj_low, adj_high, fr_low_g_list, fr_high_g_list, mlp_list):
        tdata = self.data_to_device(data)
        valid_mask = (tdata.y >= 0) & tdata.train_mask
        outputs = model(tdata, adj_low_un, adj_low, adj_high, fr_high_g_list, fr_low_g_list, mlp_list)
        if valid_mask.sum().item() == 0:
            return {'loss': torch.tensor(0.0)}  
        loss = self.criterion(outputs[valid_mask], tdata.y[valid_mask])
        return {'loss': loss}

    @torch.no_grad()
    def test(self, model, dataset, adj_low_un, adj_low, adj_high, batch_size=64, num_workers=0, fr_high_g_list=None, fr_low_g_list=None, mlp_list=None,
             pin_memory=False):
        ...
        for batch in loader:
            ...
            valid_mask = (tdata.y >= 0) & tdata.test_mask

            if fr_high_g_list is None:
                outputs = model(tdata, adj_low_un, adj_low, adj_high)
            else:
                outputs = model(tdata, adj_low_un, adj_low, adj_high, fr_high_g_list, fr_low_g_list, mlp_list)
            ...
            total_loss += num_samples * loss.item()
            total_correct += outputs[valid_mask].max(1)[1].eq(tdata.y[valid_mask]).sum().item()
            total_num_samples += num_samples

        total_loss = total_loss / total_num_samples if total_num_samples > 0 else 0
        dataset.restore_mask()
        return {'loss': total_loss, 'accuracy': 1.0 * total_correct / total_num_samples if total_num_samples > 0 else 0}
