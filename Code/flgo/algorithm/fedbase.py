    ...
    def global_test(self, model=None, flag:str='val'):
        ...
        if self.current_round == 1:
            ...
            for client in self.clients:
                client.radio_diff = 0
                client.radio_diff = calculate_shigh(client.train_data.data)
                weighted_sum += client.radio_diff * client.datavol
                total_weight += client.datavol

            weighted_average = weighted_sum / total_weight

            for client in self.clients:
                self.radio_diff.append(abs(client.radio_diff - weighted_average))
        return all_metrics
