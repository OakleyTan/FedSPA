...
from flgo.benchmark.toolkits.graph.node_classification.temp.core import GeneralCalculator, FromDatasetPipe, FromDatasetGenerator
...
class TaskPipe(FromDatasetPipe):
    ...
    def load_data(self, running_time_option) -> dict:
        ...
        for cid, cname in enumerate(self.feddata['client_names']):
            ...
            cmask = torch.zeros(self.train_data.num_nodes, dtype=torch.bool, device=device)
            cmask[cnodes] = True
            ...
            if transductive:
                task_data[cname] = {
                    'train': self.TaskDataset(cdata, 'train'),
                    'val': self.TaskDataset(cdata, 'val') if torch.any(cdata.val_mask) else None,
                    'test': self.TaskDataset(cdata, 'test') if torch.any(cdata.test_mask) else None
                }
            else:
                ...
                ctest_data = cdata
                task_data[cname] = {
                    'train': self.TaskDataset(ctrain_data, 'train'),
                    'val': self.TaskDataset(cval_data, 'val') if num_val > 0 else None,
                    'test': self.TaskDataset(ctest_data, 'test') if num_test > 0 else None
                }
        return task_data
